...
def create_index(filenames, index, file_titles):
    ...
    for filename in filenames:
        with open(filename) as file:
            ...
            file_titles[filename] = first_line.rstrip('\n')
            line_to_index(first_line, index, filename)

            for line in file:
                line_to_index(line, index, filename)

    """
    You implement this function.  Don't forget to remove the 'pass' statement above.
    """
def line_to_index(line, index, filename):
    words = line.split()
    for word in words:
    ...
    for word in words:
        if word in index:
            if ret == []:
                ...
                ret = [val for val in ret if val in index[word]]
        if ret == []:
            break
