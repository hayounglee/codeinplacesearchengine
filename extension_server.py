...
class SearchServer:
    def __init__(self):
        ...
        files = textfiles_in_dir('bbcnews')
    ...
    def handle_request(self, request):
        ...
        if request.command == 'search':
            ...
            # print(params['query'])
            found_files = search(self.index, params['query'])
            ret = []
            
            for found_file in found_files:
                ret.append({"title": self.file_titles[found_file], "date": time.ctime(os.path.getctime(found_file))})
